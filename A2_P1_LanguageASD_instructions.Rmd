...
library(pacman)
p_load(dplyr,MuMIn,lme4,pacman,lmerTest,ggplot2, magrittr)
...
#Showing gender and age distribution between ASD and TD kids
total %>% 
  dplyr::filter(VISIT ==1) %>% 
  group_by(Diagnosis) %>% 
  dplyr::summarise(amount = n(), meanage = mean(Age, na.rm = T), females = sum(Gender == "F"), males = sum(Gender == "M"))

#Showing distibution of ethnicity between ASD and TD kids
summary(total$Ethnicity[total$VISIT == 1 & total$Diagnosis == "ASD"])
summary(total$Ethnicity[total$VISIT == 1 & total$Diagnosis == "TD"])

#Showing mean and standard deviation of non-verbal IQ between ASD and TD kids
mean(total$MullenRaw[total$VISIT == 1 & total$Diagnosis == "ASD"], na.rm = T)
mean(total$MullenRaw[total$VISIT == 1 & total$Diagnosis == "TD"], na.rm = T)

sd(total$MullenRaw[total$VISIT == 1 & total$Diagnosis == "ASD"], na.rm = T)
sd(total$MullenRaw[total$VISIT == 1 & total$Diagnosis == "TD"], na.rm = T)

#Showing mean and standard deviation of verbal IQ between ASD and TD kids
mean(total$ExpressiveLangRaw[total$VISIT == 1 & total$Diagnosis == "ASD"])
mean(total$ExpressiveLangRaw[total$VISIT == 1 & total$Diagnosis == "TD"], na.rm = T)

sd(total$ExpressiveLangRaw[total$VISIT == 1 & total$Diagnosis == "ASD"])
sd(total$ExpressiveLangRaw[total$VISIT == 1 & total$Diagnosis == "TD"], na.rm = T)

#Showing mean and standard deviation of ADOS score for the ASD kids
mean(total$ADOS[total$VISIT == 1 & total$Diagnosis == "ASD"])
sd(total$ADOS[total$VISIT == 1 & total$Diagnosis == "ASD"])
```
...
```{r ex2, include = FALSE}
#Plotting data for initial overview
ggplot(total, aes(VISIT, CHI_MLU, color = Diagnosis)) + geom_point() + geom_smooth(method = "lm")

#Creating model with interaction between Visit and diagnosis
m1 <- lmer(CHI_MLU ~  VISIT*Diagnosis + (1+VISIT|SUBJ), total, REML=F) 
summary(m1)

#Creating null-model for ANOVA comparison
m2 <- lmer(CHI_MLU ~  1 + (1+VISIT|SUBJ), total, REML=F) 
...
```{r}
#Getting the fitted scores from the model and plotting them against actual MLU-values
total$fitted <- fitted(m1)
ggplot(total, aes(CHI_MLU, fitted, colour = Diagnosis)) + geom_point()
...
```{r ex3, include = FALSE}
#Plotting MLU against visit, comparing diagnoses
ggplot(total, aes(VISIT, MOT_MLU, color = Diagnosis)) + geom_point() + geom_smooth(method = "lm")

#Creating model as before, then calculating R^2
m1p <- lmer(MOT_MLU ~  VISIT*Diagnosis + (1+VISIT|SUBJ), total, REML=F) 
summary(m1p)
r.squaredGLMM(m1p)

#Creating null-model for ANOVA comparison
m2p <- lmer(MOT_MLU ~  1 + (1+VISIT|SUBJ), total, REML=F) 
summary(m2p)
...
```{r ex4, include = FALSE}
#Adding verbal IQ as predictor
full_m <- lmer(CHI_MLU ~ VISIT*Diagnosis*ExpressiveLangRaw + (1+VISIT|SUBJ), total, REML=F)
summary(full_m)
r.squaredGLMM(full_m)

#Create test model
test_m <- lmer(CHI_MLU ~ VISIT*Diagnosis+ (1+VISIT|SUBJ), total, REML=F)
summary(test_m)

#testing inclusion of verbal IQ against previous models
anova(full_m,test_m, m2)
```
